...
- [Codex] 2026-06-21 — Мобилка: `trust-bar` — скрыт список преимуществ на ≤600px;
  `cards-stack` — stack-on-scroll включён и на мобилке/планшете без счётчика;
  `pain-points` — отступ между заголовком и списком иконок доведён до 64px;
  `about` — мобильная секция с портретом перестроена без кривого наложения
  (отдельная зона фото, карточки после портрета, нормальная высота/отступы).
  Синхронизировано в `theme` и `library`, собрано, задеплоено, staging 200 — готово.
