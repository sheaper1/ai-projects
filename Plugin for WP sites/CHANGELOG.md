...
- [Claude] 2026-06-27 — **QA S10: FAQ всех страниц из драфтов клиента.** Источник —
  драфты `Новая папка (2)/*.html|.md`. Перенёс в сидеры: Home (4 Q, был дефолт),
  verkaufen (5 Q — добавлен пропущенный «…anders überlege…» + выровнены ответы),
  tippgeber (ответы Q2–Q5 → драфт), регионы (per-city FAQ в карту CITIES: Bludenz 4,
  Bregenz/Dornbirn/Feldkirch 3). Re-seed, число вопросов сверено живым HTML 1:1.
...
- [Claude] 2026-06-26 — **QA-сессия + дешёвый гейт.** Закрыто 6 секций (contact-section,
  ...
  в карточке, lead-сирота, padding заголовка). Затем — ядро НОВОГО дешёвого QA: `qa-gate.mjs`
  (числовая сверка секции с эталоном `qa-golden/`, $0 LLM, ~12с) + `qa-all.mjs` (батч-сторож
  всей библиотеки, `npm run qa:all`). Доказано: PASS на корректных, точная детекция 8 типов
  искажений без ложных, стабильно. 6 блоков заморожены → их регресс теперь бесплатен.
  Открытый хвост (НЕ закрыт, отслежен в памяти): founder-story (решение — секции нет в Figma),
  reference-catalog (плагин+контент: карусель фото карточек, ≥9 референсов, таб Gewerbe),
  property-meta/reference-related (CPT), data-pending property-hero/-stats, глоб. кнопка шапки.
...
- [Claude] 2026-06-26 — **CPT-блоки: мульти-агентный батч на 11 блоков (23 агента, 1.47M ток.).**
  ...
  ГРАБЛИ: `Workflow({name})` кэширует — ран взял старую версию без `urlOf`, всем секциям ушёл
  `URL='/'`. Агенты оказались сообразительными: сами нашли реальные CPT-страницы и сверились
  с верными Figma-нодами, правки валидны (подтвердил наложением). Урок в память
  ([[workflow-name-caches]]): после правок воркфлоу звать через `scriptPath`, не `{name}`.
  Pass: property-overview, region-services. Валидные правки задеплоены у остальных 9, но с
  выявленными ОСТАТОЧНЫМИ дефектами (веса 300→400, зазоры hero/region-hero, margin фото
  region-properties, CSS-шеврон вместо dashicons-arrow property-details, верт. выравнивание
  property-hero, stagger reference-overview, сломанная карусель property-gallery) — добить
  чистым проходом по `scriptPath` с верными URL.

- [Claude] 2026-06-26 — **tipper-form QA pass + доработан мульти-агентный флоу.** Прогнал
  блок `tipper-form` на /immobilienbewertung/ через воркфлоу `qa-page-parallel`, который
  доработал по просьбе: фаза Fix снимает наложение **ДО** фикса, Verify — **ПОСЛЕ**, всё в
  папку блока `.visual/qa/<slug>/` (figma/before-*/after-*) — видно «до vs после» за секунду;
  + отчёт о распределении (`log`), + возврат путей before/after для самопроверки оркестратором;
  `shot.mjs` научил писать в подпапку (`--name qa/<slug>/...`). Находки (подтвердил сам
  наложением, не верил «pass» вслепую): воронка разъехалась — шаг 1 «Worum geht es?» (был
  «Um was für eine Immobilie…»), 3 плитки Wohnung/Haus/Grundstück вместо 4, кнопка «Weiter →»
  (select-then-confirm), прогресс 25% — агент починил (мост на WPForms responses→summary и
  роутинг Grundstück→Liegenschaft цел). Курсив `<em>anfragen</em>` в шапке возвращён в сидере
  (plain-строка перетирала дефолт block.json) — ре-сид. qa-state pass.

- [Claude] 2026-06-26 — **Дедуп блоков: удалён мёртвый `hero`.** Аудит уникальности 51 блока:
  `hero` («первый экран») вытеснен `hero-cover` и не используется НИГДЕ (ни сидер, ни шаблон) —
  удалён из проекта (регистрация через glob(blocks/*), functions.php править не нужно; эталон
  в /library не тронут). На staging папка останется до полного деплоя — безвредно (блок нигде
  не вставлен). ОСТАЁТСЯ на новую сессию: пара `tip-form`/`tipper-form` — оба «Tipp einsenden»
  многошаговые формы (живые WPForms-мосты), консолидировать только после сверки полей, чтобы
  не сломать отправку. Остальные пересечения (how-it-works/process-steps, pain-points/problem-cards,
  8 героев, 4 CTA) — намеренные пер-контекстные варианты, не дубли.
...
- [Claude] 2026-06-26 — **Block-first QA: группа /tippgeber/ (мульти-агент, seeder-first).** Ноды
  ...
  • **how-it-works** — иконка `margin-bottom:auto`→`88px`: тело карточки стартует на Y≈199 у всех
    (короткая 2-я карточка больше не съезжает вниз, ряд тайтлов выровнен).
  • **provision-callout** — тема накидывала `margin-block:16px` на `<p>` (+64px высоты): сброс
    `margin:0` нужной специфичности → ритм 24/24/8 (h=533); убрал opacity сноски; +white-space:nowrap.
  • **tip-form** — кнопка «Weiter» по центру, лейблы muted/400, неактивные шаги opacity .5,
    точки accent; добил padding карточки 48→40, степ-лейбл r-14→r-12.
  Все сверены глазами на staging. +4 блока (итого ~20 из ~50).
