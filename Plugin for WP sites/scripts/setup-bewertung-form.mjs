// Создаёт WPForms-форму «Bewertung» (slug `bewertung`) через одноразовый Code Snippet.
...
// Запуск: node scripts/setup-bewertung-form.mjs
...
const SNIPPET = 'Library: bewertung-form setup (temporary)';

const phpCode = `
$existing = get_posts( array( 'post_type' => 'wpforms', 'name' => 'bewertung', 'post_status' => 'publish', 'numberposts' => 1 ) );
if ( ! empty( $existing ) ) { update_option( 'rosenberger_bewertung_form_ready', (int) $existing[0]->ID ); return; }
...
$post_id = wp_insert_post( array( 'post_type' => 'wpforms', 'post_title' => 'Bewertung', 'post_name' => 'bewertung', 'post_status' => 'publish', 'post_content' => '' ) );
	...
	'settings' => array(
		...
		'notifications' => array(
			1 => array(
				...
				'email' => $office,
				'subject' => 'Neue Bewertungsanfrage',
...
update_option( 'rosenberger_bewertung_form_ready', (int) $post_id );
`;
...
console.log( '🛠  Создаю WPForms-форму «Bewertung»...' );
if ( ! env.WP_URL ) throw new Error( 'нет WP_URL в .env' );

await neutralize();
const created = await api( '/wp-json/code-snippets/v1/snippets', {
	method: 'POST',
	body: JSON.stringify( { name: SNIPPET, desc: 'Одноразовое создание Bewertung-формы. Обезвреживается.', code: phpCode, scope: 'global', active: true } ),
} );
...
await neutralize();
console.log( '\n✅ Сниппет отработал и обезврежен. Форма «Bewertung» создана (или уже была).' );
console.log( '   Блок tipper-form (formSlug=bewertung) находит её по slug автоматически.' );
console.log( '   Проверь: WP Admin → WPForms → Bewertung; и страницу /immobilienbewertung/.' );
