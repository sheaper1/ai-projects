...
| Contact Section | `library/contact-section` | Контакт: слева заголовок + лид + WPForms-форма (стилизована под макет), справа Leaflet-карта с красным пином + карточка «Contact Information» (контакты из настроек сайта). Форму находит по slug `kontakt` (создаётся `scripts/setup-contact-form.mjs`), страница — `scripts/import-kontakt.mjs`. | contact, kontakt, form, map, wpforms | stable |
