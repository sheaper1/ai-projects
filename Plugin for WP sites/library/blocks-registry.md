...
| Tip Form | `library/tip-form` | «Tipp einsenden» — 3-шаговая форма по дизайну: степпер ① Das Objekt (Adresse+Objektart) → ② Die Situation (Bezug+Текст) → ③ Ihre Daten (контакты). `view.js` (статикой, не инлайн) переключает шаги + мост к скрытой WPForms по `formSlug` (default `tippgeber`). Сводка шагов 1-2 → поле Objektangaben. Используется на /tippgeber/. | tipp, form, steps, wizard, wpforms | stable |
| Tipper Form | `library/tipper-form` | Ветвящийся фаннел-опросник оценки недвижимости (~50 слайдов, `assets/funnel.html`, бренд-стиль в SCSS) + мост к скрытой WPForms (`window.RB_TIPPER`). Параметризован `formSlug`. Используется на /immobilienbewertung/ (форма `bewertung`). Грабли: инлайн-JS → wptexturize кодирует пробельный `&&`. | bewertung, funnel, опросник, wpforms | stable |
