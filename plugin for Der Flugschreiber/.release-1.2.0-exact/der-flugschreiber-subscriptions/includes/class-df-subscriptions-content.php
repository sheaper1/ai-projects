...
class DF_Subscriptions_Content
{
    ...
    public function download_protected_pdf()
    {
        ...
        if (!$post_id || !wp_verify_nonce($nonce, 'df_download_pdf_' . $post_id)) {
            wp_die(esc_html__('The download link is invalid or expired.', 'der-flugschreiber-subscriptions'), '', array('response' => 403));
        }

        $post = get_post($post_id);

        if (!$post || DF_Subscriptions::MAGAZINE_POST_TYPE !== $post->post_type || !$this->current_user_can_read_post($post)) {
            wp_die(esc_html__('You do not have access to this PDF.', 'der-flugschreiber-subscriptions'), '', array('response' => 403));
        }
        ...
        if (!$filename || !is_readable($path)) {
            wp_die(esc_html__('The PDF file could not be found.', 'der-flugschreiber-subscriptions'), '', array('response' => 404));
        }
