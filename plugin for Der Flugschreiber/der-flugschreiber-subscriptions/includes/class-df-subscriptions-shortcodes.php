...
class DF_Subscriptions_Shortcodes
{
    ...
    public function hooks()
    {
        ...
        add_shortcode('df_404', array($this, 'error_404'));
    }
    ...
    public function login_form($atts)
    {
        ...
        $header = $this->render_login_header();

        if ('lostpassword' === $action) {
            return '<div class="df-subscriptions-login-form">' . $header . $this->render_lostpassword_form() . '</div>';
        }

        if ('resetpass' === $action) {
            return '<div class="df-subscriptions-login-form">' . $header . $this->render_resetpass_form() . '</div>';
        }
        ...
        return '<div class="df-subscriptions-login-form">' . $header . ob_get_clean() . '</div>';
    }

    private function render_login_header()
    {
        ob_start();
        ?>
        <header class="df-subscriptions-login-header">
            <p class="df-subscriptions-login-header__eyebrow"><?php esc_html_e('Abonnentenbereich', 'der-flugschreiber-subscriptions'); ?></p>
            <h1><?php esc_html_e('Mein Konto', 'der-flugschreiber-subscriptions'); ?></h1>
            <p class="df-subscriptions-login-header__intro"><?php esc_html_e('Melden Sie sich an, um Ihr Abonnement und Ihre Kontodaten zu verwalten.', 'der-flugschreiber-subscriptions'); ?></p>
        </header>
        <?php
    ...
    private function render_lostpassword_form()
    {
        ...
        <?php endif; ?>
        <p class="df-subscriptions-login-intro"><?php echo esc_html($this->t('lost_intro')); ?></p>
    ...
    private function render_resetpass_form()
    {
        ...
        <?php endif; ?>
        <p class="df-subscriptions-login-intro"><?php echo esc_html($this->t('reset_intro')); ?></p>
    ...
    public function logout_link($atts)
    {
        $atts = shortcode_atts(
            array(
                'text' => __('Abmelden', 'der-flugschreiber-subscriptions'),
                'redirect' => home_url('/'),
            ),
            $atts,
            'df_logout_link'
    ...
    public function all_issues($atts)
    {
        ...
        <section class="df-issues" data-initial="<?php echo esc_attr(absint($atts['initial'])); ?>" data-step="<?php echo esc_attr(absint($atts['step'])); ?>" data-page="1">
            ...
            <div class="df-issues__controls" aria-label="<?php esc_attr_e('Ausgaben filtern', 'der-flugschreiber-subscriptions'); ?>">
                <div class="df-issues__type-toggle" role="group" aria-label="<?php esc_attr_e('Ausgabentyp', 'der-flugschreiber-subscriptions'); ?>">
    ...
    public function homepage($atts)
    {
        ...
        // Prices come from Inhalte → Startseite → Preis & Angebot. A shortcode
        // attribute still wins if one is explicitly set on the page.
        $price = '' !== $atts['price'] ? $atts['price'] : $this->t('home_price');
        $regular_price = '' !== $atts['regular_price'] ? $atts['regular_price'] : $this->t('home_regular_price');
        $issues_per_year = '' !== $atts['issues_per_year'] ? $atts['issues_per_year'] : $this->t('home_issues_per_year');
        $discount = '' !== $atts['discount'] ? $atts['discount'] : $this->t('home_discount');
        ...
        // Only feature paid issues on the homepage – free/demo PDFs are not part of the subscription pitch.
        $issues = get_posts(
            array(
        ...
        <div class="df-home">
            <section class="df-home__hero">
                <div class="df-home__hero-copy">
                    ...
                    <div class="df-home__offer">
                        <div>
                            ...
                            <span class="df-home__offer-price-row">
                                <span class="df-home__price"><?php echo esc_html($price); ?></span>
                                <?php if ($discount) : ?>
                                    <span class="df-home__offer-badge"><?php echo esc_html($discount); ?></span>
                                <?php endif; ?>
                            </span>
                        </div>
                        <div class="df-home__offer-detail">
                            <span><?php echo esc_html(sprintf($this->t('home_offer_detail'), $issues_per_year)); ?></span>
                ...
                <?php if ($latest_cover && $latest_issue) : ?>
                    <a class="df-home__hero-visual df-home__hero-cover" href="<?php echo esc_url(get_permalink($latest_issue)); ?>">
                        <span class="df-home__hero-cover-tag"><?php echo esc_html($this->t('home_current_eyebrow')); ?></span>
                        <img src="<?php echo esc_url($latest_cover); ?>" alt="<?php echo esc_attr(get_the_title($latest_issue)); ?>">
                    </a>
                <?php else : ?>
            ...
            <section class="df-home__promise" aria-label="<?php esc_attr_e('Ihre Vorteile', 'der-flugschreiber-subscriptions'); ?>">
                <div><strong><?php echo esc_html($issues_per_year); ?></strong><span><?php echo esc_html($this->t('home_promise_1_label')); ?></span></div>
            ...
            <section class="df-home__current" id="df-home-current-issue">
                ...
                <div class="df-home__current-grid">
                    <a class="df-home__current-cover" href="<?php echo esc_url($latest_issue ? get_permalink($latest_issue) : $issues_url); ?>">
                        <?php if ($latest_cover) : ?>
                    ...
                    <div class="df-home__current-copy">
                        ...
                        <p class="df-home__current-free">
                            <a href="<?php echo esc_url($issues_url); ?>"><?php echo esc_html($this->t('home_current_free')); ?> <span aria-hidden="true">&rarr;</span></a>
                        </p>
                    </div>
                </div>
            </section>
            ...
            <section class="df-home__final-cta">
                ...
                <p><?php echo esc_html(sprintf($this->t('home_cta_text'), $price)); ?></p>
                ...
                <div class="df-magazine-page__hero-grid">
                    ...
                    <div class="df-magazine-page__heading">
                        ...
                        <div class="df-magazine-page__actions">
                            ...
                            <?php endif; ?>
                            <a class="df-magazine-page__button" href="<?php echo esc_url($back_url); ?>"><?php echo esc_html($this->t('magazine_all_btn')); ?></a>
                        </div>
                        <?php if ($can_read && !$pdf_url) : ?>
                            <p class="df-magazine-page__access-note"><?php echo esc_html($this->t('magazine_access_note')); ?></p>
                        <?php endif; ?>
                    </div>
                </div>
            </header>
    ...
    public function all_articles($atts)
    {
        ...
        // Only paid issues hold individual articles – free/old PDF issues redirect
        // straight to the PDF and have no articles, so they are left out of the filter.
        $magazines = get_posts(
            array(
                'post_type' => DF_Subscriptions::MAGAZINE_POST_TYPE,
                'numberposts' => -1,
        ...
        $selected_magazine_label = $selected_magazine_id
            ...
            : __('Magazinausgabe', 'der-flugschreiber-subscriptions');
        ...
        <section class="df-articles<?php echo 'grid' === $layout ? ' df-articles--grid' : ''; ?>" data-initial="<?php echo esc_attr(absint($atts['initial'])); ?>" data-step="<?php echo esc_attr(absint($atts['step'])); ?>" data-page="1" data-layout="<?php echo esc_attr($layout); ?>">
            ...
            <div class="df-articles__filters" aria-label="<?php esc_attr_e('Artikel filtern', 'der-flugschreiber-subscriptions'); ?>">
                <?php
                echo $this->render_filter_dropdown(
                    'magazine',
    ...
    public function article_page($atts)
    {
        $atts = shortcode_atts(
            array(
                ...
                'show_back' => 'yes',
                'back_url' => '',
                'back_text' => __('Zurück', 'der-flugschreiber-subscriptions'),
                'button_text' => __('Ganzen Artikel lesen', 'der-flugschreiber-subscriptions'),
            ),
            $atts,
            'df_article_page'
        );
        ...
        $topics = is_wp_error($topics) ? array() : $topics;
        $author = $this->get_author_name($article->post_author);
    ...
    private function render_article_card(WP_Post $article)
    {
        ...
        if (!$label) {
            $label = __('Magazinartikel', 'der-flugschreiber-subscriptions');
        }
    ...
    public function error_404($atts)
    {
        $atts = shortcode_atts(
            array(
                'home_url'     => home_url('/'),
                'issues_url'   => home_url('/ausgaben/'),
                'articles_url' => home_url('/artikel/'),
        ...
        $radar = '<svg class="df-404__radar" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">'
            . '<circle cx="50" cy="50" r="47" stroke="#242424" stroke-width="2"/>'
            . '<circle cx="50" cy="50" r="31" stroke="#242424" stroke-width="1.4" opacity="0.4"/>'
            . '<circle cx="50" cy="50" r="15" stroke="#242424" stroke-width="1.4" opacity="0.4"/>'
            . '<line x1="50" y1="3" x2="50" y2="97" stroke="#242424" stroke-width="1.2" opacity="0.28"/>'
            . '<line x1="3" y1="50" x2="97" y2="50" stroke="#242424" stroke-width="1.2" opacity="0.28"/>'
            . '<path d="M24 71 Q 46 30 73 33" stroke="#242424" stroke-width="2.2" stroke-dasharray="2.5 6" stroke-linecap="round"/>'
            . '<g transform="translate(73 33) rotate(28)">'
            . '<path d="M0 -8.5 C1 -8.5 1.6 -5.5 1.6 -2.8 L8.5 1.8 L8.5 3.6 L1.6 1 L1.6 5.6 L4 7.6 L4 9.2 L0 8.2 L-4 9.2 L-4 7.6 L-1.6 5.6 L-1.6 1 L-8.5 3.6 L-8.5 1.8 L-1.6 -2.8 C-1.6 -5.5 -1 -8.5 0 -8.5 Z" fill="#242424"/>'
            . '</g>'
            . '</svg>';
        ...
        <section class="df-404">
            <div class="df-404__inner">
                <p class="df-404__eyebrow"><?php echo esc_html($this->t('e404_eyebrow')); ?></p>
                <div class="df-404__code" aria-hidden="true">
                    <span class="df-404__digit">4</span>
                    <span class="df-404__zero"><?php echo $radar; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
                    <span class="df-404__digit">4</span>
                </div>
                <h1 class="df-404__title"><?php echo esc_html($this->t('e404_title')); ?></h1>
                <p class="df-404__text"><?php echo esc_html($this->t('e404_text')); ?></p>
                <div class="df-404__actions">
                    <a class="df-404__btn df-404__btn--primary" href="<?php echo esc_url($atts['home_url']); ?>"><?php echo esc_html($this->t('e404_home')); ?></a>
                    <a class="df-404__btn" href="<?php echo esc_url($atts['issues_url']); ?>"><?php echo esc_html($this->t('e404_issues')); ?></a>
                </div>
                <div class="df-404__links">
                    <a href="<?php echo esc_url($atts['articles_url']); ?>"><?php echo esc_html($this->t('e404_articles')); ?></a>
                    <a href="<?php echo esc_url($atts['blog_url']); ?>"><?php echo esc_html($this->t('e404_blog')); ?></a>
                </div>
            </div>
        </section>
        <?php
    ...
    private function get_author_name($post_author)
    {
        $override = trim((string) get_option(DF_Subscriptions::AUTHOR_NAME_OPTION, ''));

        if ('' !== $override) {
            return $override;
        }

        return get_the_author_meta('display_name', (int) $post_author);
    }
    ...
    {
        return array(
            // Price & offer (shown in the hero, promise bar and final call-to-action)
            'home_price'             => '38,25 €',
            'home_regular_price'     => '51,00 €',
            'home_issues_per_year'   => '4',
            'home_discount'          => '-25%',
            ...
            'home_current_free'      => 'Ältere Ausgaben gratis lesen',
            ...
            'magazine_all_btn'       => 'Alle Ausgaben',
            'magazine_access_note'   => 'Diese Ausgabe ist Teil Ihres Abonnements.',
            // Article page
            ...
            // 404 page
            'e404_eyebrow'           => 'Funkstille · Fehler 404',
            'e404_title'             => 'Diese Seite ist vom Radar verschwunden',
            'e404_text'              => 'Sieht aus, als hätten wir kurz die Orientierung verloren. Die Seite, die Sie ansteuern wollten, existiert nicht (mehr) oder wurde verlegt. Kein Grund zur Sorge – zurück auf Kurs:',
            'e404_home'              => 'Zur Startseite',
            'e404_issues'            => 'Alle Ausgaben',
            'e404_articles'          => 'Alle Artikel',
            'e404_blog'              => 'Blog',
        );
    }
    ...
    private function get_login_error_message()
    {
        ...
        if ('empty' === $error) {
            return __('Bitte geben Sie Ihre E-Mail-Adresse oder Ihren Benutzernamen und Ihr Passwort ein.', 'der-flugschreiber-subscriptions');
        }

        if ('security' === $error) {
            return __('Das Anmeldeformular ist abgelaufen. Bitte versuchen Sie es erneut.', 'der-flugschreiber-subscriptions');
        }

        return __('E-Mail, Benutzername oder Passwort ist falsch. Bitte versuchen Sie es erneut.', 'der-flugschreiber-subscriptions');
    }
    ...
    public function all_posts($atts)
    {
        $atts = shortcode_atts(
            array(
                'initial' => 6,
                'step'    => 6,
                'title'   => __('Alle Beiträge', 'der-flugschreiber-subscriptions'),
                'layout'  => 'grid',
            ),
            $atts,
            'df_all_posts'
        ...
        <section class="df-articles<?php echo 'grid' === $layout ? ' df-articles--grid' : ''; ?>"
            ...
            <div class="df-articles__filters" aria-label="<?php esc_attr_e('Beiträge filtern', 'der-flugschreiber-subscriptions'); ?>">
                <?php
    ...
    public function post_page($atts)
    {
        ...
        $content     = wpautop(do_shortcode(do_blocks($raw_content)));
